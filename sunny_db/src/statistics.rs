...
mod tests {
    ...
    fn test_statistics() -> () {
        ...
        let integral = ts.integrate().unwrap();
        let definite_integral = f_nl_integrated(times[times.len() - 1] as f64) - f_nl_integrated(times[0] as f64);
        ...
        assert_eq!(ts.min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(), ts.get_current_values().last().unwrap().1);
